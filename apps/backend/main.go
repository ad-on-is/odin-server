...
func getDeps() echo.MiddlewareFunc {
...
func main() {
	...
	app.OnBeforeServe().Add(func(e *core.ServeEvent) error {
		...
		e.Router.GET("/-/refreshHistory", func(c echo.Context) error {
			info := apis.RequestInfo(c)
			id := info.AuthRecord.Id

			q := app.Dao().DB().Delete("history", dbx.NewExp("user = {:user}", dbx.Params{"user": id}))
			q.Execute()

			trakt.SyncHistory(id)

			// q := app.Dao().DB().NewQuery("DELETE FROM history WHERE user = {:user}").Bind(dbx.Params{"user": id})
		...
		e.Router.GET("/-/health", func(c echo.Context) error {
			...
			tr, _, _ := trakt.CallEndpoint("/users/settings", "GET", types.TraktParams{Headers: theaders})
			return c.JSON(http.StatusOK, map[string]any{"realdebrid": rd, "alldebrid": ad, "trakt": tr, "version": getVersion(), "date": date})
		}, RequireDeviceOrRecordAuth(app))
